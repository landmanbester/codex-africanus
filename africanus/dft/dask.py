...
from africanus.dft.kernels import im_to_source_vis as np_im_to_source_vis
...
def _im_to_source_vis_wrapper(image, uvw, lm, frequency, dtype_):
    return np_im_to_source_vis(image, uvw[0], lm[0],
                               frequency, dtype=dtype_)


@requires_optional('dask.array', dask_import_error)
def im_to_source_vis(image, uvw, lm, frequency, dtype=np.complex128):
    """ Dask wrapper for im_to_vis function """
    if lm.chunks[0][0] != lm.shape[0]:
        raise ValueError("lm chunks must match lm shape "
                         "on first axis")
    if image.chunks[1][0] != image.shape[1]:
        raise ValueError("Image chunks must match image "
                         "shape on second axis")
    if image.chunks[1][0] != lm.chunks[0][0]:
        raise ValueError("Image chunks must match lm chunks "
                         "on second axis")
    if image.chunks[0] != frequency.chunks[0]:
        raise ValueError("Image chunks must match frequency "
                         "chunks on second axis")
    return da.core.blockwise(_im_to_source_vis_wrapper, ("row", "chan", "source", "corr"),
                             image, ("chan", "source", "corr"),
                             uvw, ("row", "(u,v,w)"),
                             lm, ("source", "(l,m)"),
                             frequency, ("chan",),
                             dtype=dtype,
